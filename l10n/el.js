OC.L10N.register(
    "biblio",
    {
    "Biblio" : "Biblio",
    "A library management system" : "Σύστημα διαχείρισης βιβλιοθηκών",
    "This can be used to manage a book library, Sheet music archive, Textbook library" : "Αυτό μπορεί να χρησιμοποιηθεί για τη διαχείριση μιας βιβλιοθήκης, ενός αρχείου μουσικής, μιας βιβλιοθήκης σχολικών βιβλίων",
    "Could not create the medium" : "Αδυναμία δημιουργίας του μέσου",
    "Could not update the medium" : "Αδυναμία ενημέρωσης του μέσου",
    "Medium deleted" : "Το μέσο διαγράφηκε",
    "Could not delete the medium" : "Αδυναμία διαγραφής του μέσου",
    "Delete entry" : "Διαγραφή καταχώρησης",
    "Drag to reorder the fields" : "Σύρετε για αναδιάταξη των πεδίων",
    "Title" : "Τίτλος",
    "A title is required!" : "Απαιτείται ένας τίτλος!",
    "Delete Field" : "Διαγραφή πεδίου",
    "Add a new entry" : "Προσθήκη νέας καταχώρησης",
    "Value" : "Τιμή",
    "Lend/Return" : "Δανεισμός/Επιστροφή",
    "Mediums" : "Μέσα",
    "Customers" : "Πελάτες",
    "Add a field" : "Προσθήκη πεδίου",
    "Save" : "Αποθήκευση",
    "New Medium" : "Νέο μέσο",
    "List" : "Λίστα",
    "Short text" : "Σύντομο κείμενο",
    "Enter a short text" : "Εισαγωγή σύντομου κειμένου",
    "Long text" : "Εκτεταμένο κείμενο",
    "Enter a long text" : "Εισαγωγή εκτεταμένου κειμένου",
    "Date" : "Ημερομηνία",
    "Pick a date" : "Επιλογή ημερομηνίας",
    "Date and time" : "Ημερομηνία και ώρα",
    "Pick a date and time" : "Επιλογή ημερομηνίας και ώρας",
    "Could not create medium" : "Αδυναμία δημιουργίας μέσου",
    "Could not fetch mediums" : "Αδυναμία λήψης μέσων",
    "Could not update title" : "Αδυναμία ενημέρωσης τίτλου"
},
"nplurals=2; plural=(n != 1);");
